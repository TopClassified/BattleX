...
// 轴经"+添加状态轴/+添加行为轴"(GameplayTag选择器)添加,点击行头/列头删除对应轴;
...
// 行头显示状态Tag时省略命名空间首段(BXStunState.Knockback → Knockback),列头省略 BXBehavior. 前缀
...
private:
	...
	// 状态轴显示名(省略命名空间首段:BXStunState.Knockback → Knockback;悬停提示保留完整名)
	...
	// 添加轴(弹出GameplayTag选择器;bInStateAxis=true加状态行,false加行为列)
	FReply OnAddAxisClicked(bool bInStateAxis);

	// 删除状态行轴(连带清除该状态行的全部关系配置)
	FReply OnRemoveStateAxisClicked(int32 InAxisIndex);

	// 删除行为列轴(连带清除各状态行容器中对该行为的引用)
	FReply OnRemoveBehaviorAxisClicked(int32 InAxisIndex);
