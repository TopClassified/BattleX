...
#define LOCTEXT_NAMESPACE "TimeSliderController"
...
FTimeSliderController::FTimeSliderController
(
	const FTimeSliderArgs& InArgs, TWeakPtr<FTimelineController> InEditorTimelineController, 
	TWeakPtr<STimeline> InWeakTimeline, TSharedPtr<INumericTypeInterface<double>> InSecondaryNumericTypeInterface
...
FTimeSliderController::FScrubPixelRange FTimeSliderController::GetScrubberPixelRange(FFrameTime ScrubTime, FFrameRate Resolution, FFrameRate PlayRate, const FScrubRangeToScreen& RangeToScreen, float DilationPixels) const
{
	...
	if (Range.bClamped)
	{
		Range.HandleRange = TRange<float>((StartPixel + EndPixel - MinScrubSize) * .5f, (StartPixel + EndPixel + MinScrubSize) * .5f);
	}
	else
	{
...
int32 FTimeSliderController::OnPaintTimeSlider(bool bMirrorLabels, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (LocalSequenceLength > 0)
	{
...

		{
			...
			FVector2f TextSize = FontMeasureService->Measure(FrameString, SmallLayoutFont);
			...
			FVector2f TextOffset(TextPosition, Args.bMirrorLabels ? TextSize.Y - 6.f : Args.AllottedGeometry.Size.Y - (Args.MajorTickHeight + TextSize.Y));
...
FReply FTimeSliderController::OnMouseButtonUp(SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	else if (bHandleLeftMouseButton)
	{
		...
		else
		{
			...
			CommitScrubPosition(MouseTime, false);
		}
	...
	else if (bHandleLeftMouseButton)
	{
		...
		if (MouseDragType == DRAG_NONE)
		{
			if (DistanceDragged > FSlateApplication::Get().GetDragTriggerDistance())
			{
				...
				const bool bHitTime = HitTimeIndex != INDEX_NONE;
...
void FTimeSliderController::CommitScrubPosition(FFrameTime NewValue, bool bIsScrubbing)
{
	...
	TimeSliderArgs.OnScrubPositionChanged.ExecuteIfBound(NewValue, false, bIsScrubbing);
}
...
FCursorReply FTimeSliderController::OnCursorQuery(TSharedRef<const SWidget> WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	...
	const bool bHitTime = (HitTestTimes(RangeToScreen, HitTestPixel) != INDEX_NONE);
...
TSharedRef<SWidget> FTimeSliderController::OpenSetPlaybackRangeMenu(FFrameNumber FrameNumber)
{
	...
	MenuBuilder.BeginSection("AnimSelectionRangeMenu", FText::Format(LOCTEXT("SelectionRangeTextFormat", "Selection Range ({0}):"), CurrentTimeText));
	{
		...
		(
			...
			(
				...
				FCanExecuteAction::CreateLambda([=,this]{ return SelectionRange.IsEmpty() || FrameNumber < UE::MovieScene::DiscreteExclusiveUpper(SelectionRange); })
			)
		);
...
void FTimeSliderController::ClampViewRange(double& NewRangeMin, double& NewRangeMax)
{
...
bool FTimeSliderController::HitTestRangeStart(const FScrubRangeToScreen& RangeToScreen, const TRange<double>& Range, float HitPixel) const
{
	if(Range.HasLowerBound())
	{
		...
		return HitPixel >= RangeStartPixel - MouseTolerance - DragToleranceSlateUnits && HitPixel <= RangeStartPixel + MouseTolerance + BrushSizeInStateUnits + DragToleranceSlateUnits;
	}
...
bool FTimeSliderController::HitTestRangeEnd(const FScrubRangeToScreen& RangeToScreen, const TRange<double>& Range, float HitPixel) const
{
	if(Range.HasUpperBound())
	{
		...
		return HitPixel >= RangeEndPixel - MouseTolerance - BrushSizeInStateUnits - DragToleranceSlateUnits && HitPixel <= RangeEndPixel + MouseTolerance + DragToleranceSlateUnits;
	}

	return false;
}

bool FTimeSliderController::HitTestTime(const FScrubRangeToScreen& RangeToScreen, double Time, float HitPixel) const
{
	...
	return HitPixel >= TimePixel - MouseTolerance - HalfBrushSizeInStateUnits - DragToleranceSlateUnits && HitPixel <= TimePixel + MouseTolerance + HalfBrushSizeInStateUnits + DragToleranceSlateUnits;
}

void FTimeSliderController::SetPlaybackRangeStart(FFrameNumber NewStart)
{
	TRange<FFrameNumber> PlaybackRange = TimeSliderArgs.PlaybackRange.Get();
...
void FTimeSliderController::SetPlaybackRangeEnd(FFrameNumber NewEnd)
{
	TRange<FFrameNumber> PlaybackRange = TimeSliderArgs.PlaybackRange.Get();
...
void FTimeSliderController::SetSelectionRangeStart(FFrameNumber NewStart)
{
	TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
...
void FTimeSliderController::SetSelectionRangeEnd(FFrameNumber NewEnd)
{
	TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
...
int32 FTimeSliderController::HitTestTimes(const FScrubRangeToScreen& RangeToScreen, float HitPixel) const
{
	return INDEX_NONE;
}
