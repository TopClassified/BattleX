...
- **`BXBehavior_Root` 显式定义已移除（2026-09-16 用户拍板）**：层级由字符串天然构成（隐式父节点同样入 GameplayTagNodeMap，`RequestGameplayTag`/`RequestGameplayTagChildren` 对隐式节点有效，已对照 5.8 引擎源码 InsertTagIntoNodeArray 验证）；矩阵自动补齐树根与显示名前缀剥离改用字面量 `"BXBehavior"`/`"BXBehavior."`。**代价（用户知情接受）**：蓝图/资产侧 Tag 选择器只列显式 Tag，从此选不到 `BXBehavior` 根——族级全禁配置无入口，需逐个列具体行为 Tag。
- **状态 Tag 树两级化（2026-09-16 二次迁移）**：`BXState.*` 平铺升级为 **`BXState.Stun.*`**（宏 BXState_Stun_X；坠落中=AirFall 避让行为名 Falling；中文注释=无硬直/被击退/被击倒/被击趴/上飞中/坠落中/滞空/被处决）；矩阵根串硬编码 `BXState.Stun`（SBXStateRelationMatrix::EnsureAxesComplete）。行头显示省略首段（BXState.Stun.Knockback→Stun.Knockback）。
...
- **插件 ini 直写必须先 `GConfig->LoadFile(路径)`（2026-09-17 根因修复，三次数据损毁定位）**：`SaveConfig(CPF_Config, 文件路径)` 对配置系统里**无 branch 的文件走"临时 branch"路径**——`AddNewBranch` 创建**空内存文件且从不加载磁盘已有内容**，属性逐个 Set 进空文件后 `Flush→SaveBranch→WriteToString` **整体覆写目标文件，其余节全部丢失**（文件头出现 `;METADATA=(Diff=true, UseCommands=true)` 即其指纹；引擎源码链：Obj.cpp `UObject::SaveConfig` 临时 branch 判定 + ConfigCacheIni.cpp `AddNewBranch`/`SaveBranch`）。此 bug 单节文件时代不可见，两个关系类共用插件 ini 后交叉覆写暴露。修复：两个 `SaveToPluginConfig` 在 SaveConfig 前先 `GConfig->LoadFile(PluginIniPath)` 把完整磁盘文件注册为常驻 branch，SaveConfig 随后走差量 Set、Flush 保留全部节。**任何新的"直写特定 ini 文件"代码都必须遵守此模式**。
